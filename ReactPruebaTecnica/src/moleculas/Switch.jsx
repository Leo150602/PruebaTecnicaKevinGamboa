...
const Switch = () => {
  let { modoOscuro, AlternarTema } = UsarTema(); 
  
  
  return (
    <StyledWrapper>
      <label htmlFor="themeToggle" className="themeToggle st-sunMoonThemeToggleBtn" type="checkbox">
        <input type="checkbox" id="themeToggle" className="themeToggleInput" checked={modoOscuro} onChange={AlternarTema} />
        <StyledSVG width={28} height={28} viewBox="0 0 20 20" fill="currentColor" stroke="none" $modoOscuro={!modoOscuro}>
          <mask id="moon-mask">
...
const StyledSVG = styled.svg`
  color: ${({ $modoOscuro }) => ($modoOscuro ? "white" : "black")};
